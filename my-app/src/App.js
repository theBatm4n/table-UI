...
const App = () => {
  ...
  // for keeping track of colour i.e colorIndex[i] is the index for the ith topic in selectedTopics array
  ...
  const handleClick = useCallback(
    (value) => {
      if (!selectedTopics.includes(value)) {
        ...
        let i = 0
        while(colorIndex.includes(i)){
          i++;
        }
        setcolorIndex([...colorIndex, i])
      } else {
        const updatedTopics = selectedTopics.filter((topic) => topic !== value); // deselect the selected topic
        const colorindextoremove = selectedTopics.indexOf(value);
        setSelectedTopics(updatedTopics);
        if(colorindextoremove !== -1){
          const updatedcolorindex = colorIndex.filter((_, index) => index !== colorindextoremove)
          setcolorIndex(updatedcolorindex)
        }
      }
    },
    [selectedTopics, colorIndex]
  );
  ...
  const rowData = useMemo(
    () => [
      {
        ticker: "#TCS.NS",
        segment: "#HotelDivision",
        metric: "#SegmentRevenue_AsReported",
        Date: "2024-09-11",
        Timeperiod: "3M ",
        Unit: "INR",
        Value: "19.19m",
      },
      {
        ticker: "#CANDC.NS",
        segment: "#ITServices",
        metric: "#SegmentAssets_AsReported",
        Date: "2024-09-15",
        Timeperiod: "1Y ",
        Unit: "USD",
        Value: "5.67b",
      ...
      },
      {
        ticker: "#CYBERMEDIA.NS",
        segment: "#Media",
        metric: "#SegmentLiabilities_AsReported",
        Date: "2024-09-30",
        Timeperiod: "1Y ",
        Unit: "USD",
        Value: "15.75b",
  ...
  const colDefs = useMemo(
    ...
    [handleClick, selectedTopics, colorIndex]
  );
